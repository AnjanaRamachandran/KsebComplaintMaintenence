...
public class EditAllocation extends HttpServlet{
	...
	try {
		...
		out.print("<html>");
		out.print("<head><link rel='stylesheet' type='text/css' href='css/editworkallocation.css'></head>");
		out.print("<body>");
		
		
		
		out.print("<div class='welcomeadmin'>");
		out.print("<a href='complaint'>Complaint Registration</a>");
		out.print("<a href='viewcomplaint'>View Complaints </a>");
		...
		pst = con.prepareStatement("select allocation_id,allocation_date,complaint_id,lineman from work_allocation where allocation_id=?");
		pst.setInt(1, id);
		rs=pst.executeQuery();
		rs.next();
		
		
		 PreparedStatement pst2 = con.prepareStatement("SELECT user_name FROM user_login where user_type='LineMan'");
            ResultSet rs2 = pst2.executeQuery();
            rs2=pst2.executeQuery();
		
		out.print("<h1>Edit Work Allocation </h1>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		con = dbcon1.getConnection();
		pst1 = con.prepareStatement("update work_allocation set allocation_date=?,complaint_id=?,lineman=? where allocation_id=?");
